...
// Build an Activity.Info for one sample. CarbBurnView.compute() overrides the
// typed DataField.compute(info as Activity.Info), so the type checker requires
// a real Activity.Info (a duck-typed stub is rejected). compute() reads only
// currentPower / timerTime / calories; the rest stay null.
function mkInfo(power, tMs, cal) as Toybox.Activity.Info {
    var info = new Activity.Info();
    info.currentPower = power;   // Number or null (null/<=0 => coast)
...
function test_dt_invariance(logger) {
    ...
    v1.compute(mkInfo(P, 18000, null));          // one dt=2 step
    v2.compute(mkInfo(P, 17000, null));          // two dt=1 steps
    v2.compute(mkInfo(P, 18000, null));
...
function test_coast_brief_dropout_and_boundary(logger) {
    ...
    var t = 21000;
    t += 1000; v.compute(mkInfo(null, t, null)); // coast 1 (mCoastN=1)
    t += 1000; v.compute(mkInfo(null, t, null)); // coast 2 (mCoastN=2 < COAST_HOLD_N=3)
    var heldAtN1 = cbvFloatEq(v.mCarbPctRoll, pctBefore, 1e-9)
                   && (v.zoneColor(Graphics.COLOR_LT_GRAY, true) == Graphics.COLOR_RED)
                   && (v.mCoastN == 2);
    t += 1000; v.compute(mkInfo(null, t, null)); // coast 3 (mCoastN=3 >= N) -> % decays
...
function test_null_power_no_crash(logger) {
    var v = new CarbBurnView();
    v.compute(mkInfo(null, 1000, null));         // prime, coast, null power
    v.compute(mkInfo(null, 2000, null));         // coast, dt=1, null power
