...
- **orange** — 50%-carb crossover up to the 80%-carb power
- **red** — above the 80%-carb power (carbohydrate almost entirely dominant)

All boundaries are derived from your own thresholds (the fat-max band, the 50%-carb
crossover, and the 80%-carb power), so they scale per rider. For the generic sample
...
| 50%-carb → 80%-carb | 195–239 W | orange |
| Above 80%-carb | > 239 W | red |

The 80%-carb power sits just below FTP, so **red flags threshold-and-harder efforts**
(where you're burning carbohydrate about as fast as the body can). The smoothed power
(not the instantaneous value) drives the color, so it transitions cleanly rather than
flickering on surges.
